...
public class SubAppMeetingService {
	 ...
	 * 
	 * @methodDesc:  
	 * 查询会议信息,超级管理员可以查看所有会议，其他管理员只能查看自己创建的会议
	 * @param subAppMeeting
	 * @param fnfhPageData
	 * @param loginUser
	 * @return
	 ...
	 */
	public Result findsubAppMeeting(SubAppMeeting subAppMeeting,FnfhPageData fnfhPageData,
			User loginUser) throws FnfhException{
	 ...
	 */
	public Result addOrModifySubAppMeeting(SubAppMeeting subAppMeeting,
			FnfhPageData pgdata,User loginUser, Integer attendeesLimit) throws FnfhException{
		int result=0;
		try {
...
//					userList=getUserList(userList);
    ...
    public Result findMeetingsOfAttendee(FnfhPageData pageData, User loginUser)
    {
        ...
        List<SubAppMeeting> subAppMeetingList = this.subAppMeetingMapper
            .findSubAppMeeting(subAppMeeting);
        ...
        if (subAppMeetingList != null && !subAppMeetingList.isEmpty())
        {
            for (SubAppMeeting meeting : subAppMeetingList)
            {
                ...
                List<SubAppMeetingFile> fileList = this.subAppMeetingFileMapper
                    .findSubAppMeetingFile(meetFile);
    ...
    public Result updatedMeetingAndWriteFile(String mid, Integer is_ipad, List<String> driverLetterList, HttpServletRequest request) throws FnfhException
    {
       ...
       // int count = subAppMeetingMapper.findSubAppMeetingCount(meeting);
        ...
        if (driverLetterList != null && !driverLetterList.isEmpty())
        {
            ...
            //获取会议文件
            List<SubAppMeetingFile> fileList = this.subAppMeetingFileMapper
                .findSubAppMeetingFile(new SubAppMeetingFile(meeting.getMid()));
            meeting.setFileList(fileList);
       ...
       // int count = subAppMeetingMapper.findSubAppMeetingCount(meeting);
        ...
        for (int i = 0; i < devices.length; i++)
        {
            ...
            if (canPushMeetToDevice(fileManager, devices[i], meeting))
            {
                ...
                meetingInfoArray.add(meetingInfoMap);
                File jsonFile = new File(System.getProperty("shenhua.root") + File.separator + "tmp"
                    + File.separator + System.currentTimeMillis() + File.separator + "meetingInfo.json");
                try
                {
                    FileUtils.writeStringToFile(jsonFile, meetingInfoArray.toString(), "UTF-8");
                    //将jsonFile 发送到设备中
        ...
        if (meetingFileList != null)
        {
            ...
            if (fileCount > 1)
            {
                ...
                for (SubAppMeetingFile file : meetingFileList)
                {
                    ...
                    {
                        //如果合并的会议文件不是最新的
                        if ( file.getLast_update_time()
                            .compareTo(lastMergeFileMap.get("last_update_time").toString()) < 0)
                        {
                            continue;
                        }
                        else

...
var idNameArr=["","admin","ipad","meet","usermanage"];
var treeIdNameArr=["","treeDemo","treeDemo2","treeDemo3","treeDemo4"];
var checkBoxIdArr=["","includeChild","includeChild2","includeChild3","includeChild4"];
var keywordsIdArr=["","keywords","keywords2","keywords3","keywords4"];
var treeId="treeDemo";
...
function createTree () {
	...
	else if(currentPerSubType==4){isCreate4=true;}
      var zNodes;
      $.ajax({
...
//        	var adminList=result.data.adminlist;
        	...
        	var usermanageList=result.data.usermanagelist;
        	var html="";
        	var names="";
        	var ids="";
        	//模块管理员
...
//        	$("#adminIds").val(ids);
        	...
        	names="";
        	ids="";
        	$.each(usermanageList, function (index, item) {    
        		html+='<option value="'+item.user_id+'">'+item.real_name+'</option>'; 
        		names+=item.real_name+";";
        		ids+=item.user_id+";";
            });
        	$("[name='usermanageselectms2side__dx[]']").html(html);
        	$("#usermanageNames").val(names);
        	$("#usermanageIds").val(ids);
        } 
        else{
        	layer.alert(result.message);
        }
...
//	        				goBack();
	...
	var bindEvent=function()
	{
		...
		$("#selectUserManageBtn").click(showSelectUserManageLayer);

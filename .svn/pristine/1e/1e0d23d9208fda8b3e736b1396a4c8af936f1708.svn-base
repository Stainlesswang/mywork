...
public class WebTools 
{
	...
	private static String nginxiptag="X-Real-IP";
	private static String nginxforwardtag="x-forwarded-for";
    ...
    {
        String ip = request.getHeader(nginxforwardtag);
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip))
        {
            ip = request.getHeader(nginxiptag);
        }
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip))
        {

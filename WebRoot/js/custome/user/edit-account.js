		...
		if($.trim(user_id)!='')
		{
			...
			var succeed = function (result) {
	            if (0 == result.status) {
					...
					$("#user_name").hide();
					if(userInfo.is_super=="1")//如果是超级管理员
					{
						$("#selectOrgBtn").hide();
					...
					else
					{
						...
						$("input[name='_admin_type']").attr("disabled",true);
					}
	            } else {
...
//	     });
	...
	var checkValidate=function()
	{
		...
		if(flag&&($("#is_super").val()!="1"))
		{
			flag=validate_notNull($("#org_name"),"#org_name","组织不能为空");
		}
		...
		if(flag)
		{
			...
			{
				showTips("3-16位，字符、数字和下划线。","#user_name");
				return false;
			}
